<template>
    <div>
        <div>
            <div class="text-xs font-hairline text-gray-600 pb-1">{{eveDate}}, {{eveTime}}</div>
        </div>
        <div class="mb-10 flex justify-between pt-3 border-t-2 border-green-600">
            <div class="flex-shrink-0">
              <div class="text-xs font-hairline text-gray-600">{{eventData.entry}} $</div>
            </div>
            ...
            <div class="flex-shrink-0">
                <div>
                    <router-link :to="{ name: 'edit-event', params: { eventId: eventData.eventId }}" class="capitalize cursor-pointer text-xs font-hairline text-gray-600 outline-none leading-none hover:text-green-600">edit</router-link>
                </div>
                <div>
                    <span @click="deleteEvent" class="capitalize cursor-pointer text-xs font-hairline text-gray-600 outline-none leading-none hover:text-red-600">delete</span>
                </div>
            </div>
        </div>
    </div>
